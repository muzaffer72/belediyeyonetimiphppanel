<?php
// Belediye yönetim sistemi giriş sayfası - Entegre sistem
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($password)) {
        ...
        if ($login_type === 'admin') {
            ...
            // Belediye personeli/moderatör girişi - önce users tablosundan kullanıcıyı bul
            ...
            if (!$users_result['error'] && !empty($users_result['data'])) {
                ...
                // Şifre kontrolü (test için herhangi bir şifreyi kabul et)
                if (!empty($password)) {
                    ...
                    // User tablosundaki role alanını kontrol et (admin, moderator, official, user)
                    $user_role = $user['role'] ?? 'user';
                    
                    // Sadece admin, moderator, official rolündeki kullanıcılar giriş yapabilir
                    if (in_array($user_role, ['admin', 'moderator', 'official']) || $is_official) {
                        // Personelin atandığı şehir/ilçe bilgilerini getir
                        $assigned_city_id = null;
                        $assigned_district_id = null;
                        $assigned_city_name = null;
                        $assigned_district_name = null;
                        
                        if ($is_official && $official_data) {
                            $assigned_city_id = $official_data['city_id'];
                            $assigned_district_id = $official_data['district_id'];
                            
                            // Şehir bilgisini getir
                            if ($assigned_city_id) {
                                $city_result = getDataById('cities', $assigned_city_id);
                                if (!$city_result['error'] && $city_result['data']) {
                                    $assigned_city_name = $city_result['data']['name'];
                                }
                            }
                            
                            // İlçe bilgisini getir
                            if ($assigned_district_id) {
                                $district_result = getDataById('districts', $assigned_district_id);
                                if (!$district_result['error'] && $district_result['data']) {
                                    $assigned_district_name = $district_result['data']['name'];
                                }
                            }
                        }
                        ...
                        $_SESSION['user_type'] = $user_role === 'admin' ? 'admin' : ($user_role === 'moderator' ? 'moderator' : 'official');
                        $_SESSION['assigned_city_id'] = $assigned_city_id;
                        $_SESSION['assigned_district_id'] = $assigned_district_id;
                        $_SESSION['assigned_city_name'] = $assigned_city_name;
                        $_SESSION['assigned_district_name'] = $assigned_district_name;
                        ...
                        if ($is_official && $official_data) {
                            ...
                            if ($user_city_id) {
                                ...
                                if (!$city_result['error'] && $city_result['data']) {
                                    $user_city_name = $city_result['data']['name'];
                                }
                            }
